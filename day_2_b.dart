...
main() {
  ...
  int numBadPasswords = 0;

  // take an element from passwords
  for (String inputLine in passwords) {
    // parse out rules from password
    List<String> parts = inputLine.split(': ');
    String ruleString = parts[0];
    String password = parts[1];
    
    // interpret rules
    // rule is: min int, '-', max int, ' ', letter
    List<String> ruleParts = ruleString.split(' ');
    String rangeString = ruleParts[0];
    String ruleChar = ruleParts[1];

    // we now have a ruleset
    List<String> rangeParts = rangeString.split('-');
    int firstPosition = int.parse(rangeParts[0]) - 1;
    int secondPosition = int.parse(rangeParts[1]) - 1;

    // count the correct instances based on ruleset
    int numInstances = 0;
    bool isFirstPositionMatch = password[firstPosition] == ruleChar;
    bool isSecondPositionMatch = password[secondPosition] == ruleChar;

    // tally correct passwords
    // TODO: there's a better way to check booleans, but dart had an issue with compound checks, so simplifying...
    if (isFirstPositionMatch && isSecondPositionMatch) {
      numBadPasswords += 1;
      continue;
    } // both of them
  ...
  // print results
  print('${passwords.length} passwords, $numGoodPasswords good passwords, $numBadPasswords bad passwords.');
}